...
Added 2026-09-21, from the whole-branch review's fix wave. These are recorded rather than fixed: each is a design decision that belongs to a later milestone, not a defect this wave should have changed unilaterally.

7. **The band used for calibration is not the band the detector was scored on.** `_worst_band` runs over ALL observations; each detector separately drops observations below its own floor via `filter_by_quality_floor`. Different sets. Demonstrated by execution: 31 `high` frames plus one flat `reject` frame, a detector with floor `low`, a calibrator fitted on `high` — the detector scores the 31 high frames and returns raw 0.3135, `decide` calibrates that on `reject` (a frame the detector never saw) and the evidence collapses to `{"llr": 0.0, "abstained": true, "reason": "uncalibrated_for_band"}`. The same clip without the one flat frame yields `llr -0.713, reason "ok"` from the identical raw score. **One bad frame in thirty-two destroys the sample's entire evidence**, and the realistic outcome is not "pessimistic calibration" but NO calibration, because no `reject`-band curve will ever be fitted — detectors abstain there by design. Invisible today only because nothing is calibrated at all; live the first day a fitted calibrator lands. The design spec's §4.2 cost statement has been corrected; the code's band semantics are deliberately unchanged and must be re-argued at the calibration milestone.
8. **`Detector.modalities` is declared by every detector and consumed by nothing.** `decide` scores every registered detector regardless of `sample.modality`, so an image-only detector is still asked to score a video.
9. **`normalize` never supplies yaw or pitch.** `measure_quality` is called without them, so `Quality.yaw_deg` and `Quality.pitch_deg` are `0.0` in every record ever produced, and `quality.MAX_YAW_HIGH` is permanently inert — a profile face bands identically to a frontal one.
10. **Per-detector latency is captured only to a DEBUG log.** `decide` times each `score` call and logs it; nothing returns it. Acceptance criterion 5 (p95 latency on the target hardware) has no path out of `decide`.
11. **`src/dfd/ingest/base.py`'s `IngestAdapter` Protocol is dead code.** Nothing implements or references it, and its signature — `(path, context)` — matches neither `load_image(path, context, limits)` nor `load_video(path, context, max_frames, seed, limits)`. It reports 0% coverage.
